 ...
 *
 * Devots and monsters share almost nothing in the simulation — one has a god, a
 * line, items and reproduction; the other has hunger and a hoard. They meet
 * here, at the one place where both are simply "something that thinks".
 */
...
export function devotSubject(devot: DevotEntity): ThoughtSubject {
  return {
    ...
    name: devot.name,
    pos: devot.pos,
    balance: devot.balance,
    capacity: devot.capacity,
    state: devot.state,
    age: devot.age,
    traits: devot.traits,
    isFounder: devot.isFounder,
    identityJson: devot.identityJson,
    items: devot.items,
  };
}
