...
/**
 * Schema for a FRESH database. `CREATE TABLE IF NOT EXISTS` is a no-op on a
 * file that already exists, so this DDL alone can never evolve an old database:
 * every change made after the first release also needs a MIGRATIONS entry below.
 */
const DDL = `
